...
<p align="center"><a href="https://ayuvo-health.web.app">Website</a> · <a href="CONTRIBUTING.md">Contribute</a> · <a href="SECURITY.md">Security</a> · <a href="LICENSE">MIT licence</a></p>

Private, open-source, all-in-one health companion for iPhone (SwiftUI) and Android (Jetpack Compose): nutrition, workouts, health records, medications, a local mirror of Apple Health / Health Connect, fasting, water and an AI coach that runs on the API key the user brings, or on a model that stays on the phone. No account, no Ayuvo servers, no analytics.

> **Inspired by [Fud AI](https://github.com/apoorvdarshan/fud-ai)** by Apoorv Darshan. Ayuvo started from that MIT-licensed calorie tracker; its copyright notice is kept in [`LICENSE`](LICENSE) and shown in the apps under Settings → Legal → Licenses.
...
| **AI coach** | Multi-turn chat with tool calls over the user's own data (diary, workouts, weight trend, fasts, and Health data only behind a visible consent toggle). 15 BYOK providers, or on-device models (Gemma 4 E2B, Qwen3 and Google's **MedGemma 1.5 4B**, a gated research model under the Health AI Developer Foundations terms, not a medical device) / Apple Intelligence; see `local-models/README.md`; keys in Keychain / EncryptedSharedPreferences. |
| **Platform** | Apple Watch app + complications, iOS/Android widgets, Siri Shortcuts, iOS Share Extension, 18 languages, 18 accent tints with matching icons, Export/Import All Data zip on both platforms, which also moves a profile and its logs between iPhone and Android (`docs/portable-data.md`), plus optional Google Drive backup on Android (the Drive backup never includes health data or coach chat). |
...
├── web/            # static site (Firebase Hosting): landing, feature pages, privacy, terms, support. Generated by scripts/web_build.py
...
├── scripts/web/    # site content + components, screenshot redaction, social cards (scripts/web_build.py builds web/)
...
Ayuvo is open source at [github.com/astechtic/ayuvo](https://github.com/astechtic/ayuvo). Code, translations, exercise data and docs are welcome: see [`CONTRIBUTING.md`](CONTRIBUTING.md). Report vulnerabilities privately as described in [`SECURITY.md`](SECURITY.md).

## Licence

MIT — see `LICENSE`. Ayuvo is inspired by, and built on, the MIT-licensed [Fud AI](https://github.com/apoorvdarshan/fud-ai) code base; the original copyright notice is retained there and shown in the apps under Settings → Legal → Licenses. Third-party credits: `THIRD_PARTY_NOTICES.md`, `ASSET_CREDITS.md`, `local-models/legal/`.
