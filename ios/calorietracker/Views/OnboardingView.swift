...
struct OnboardingView: View {
    ...
    @State private var step = OnboardingView.initialStepFromLaunchArguments()
    ...
    // The cloud form never holds `.gemma4Local`; the on-device model is its own option.
    @State private var byokProvider: AIProvider = OnboardingView.initialCloudProvider()
    @State private var byokModel: String = OnboardingView.initialCloudModel()
    @State private var byokApiKey: String = AIProviderSettings.apiKey(for: OnboardingView.initialCloudProvider()) ?? ""
    @State private var byokBaseURL: String = AIProviderSettings.customBaseURL(for: OnboardingView.initialCloudProvider()) ?? ""
    ...
    private static func initialAISetupMode() -> AISetupMode {
        AIProviderSettings.selectedProvider == .gemma4Local || AIProviderSettings.pendingLocalModelSelection
            ? .onDevice : .cloud
    }

    private static func initialCloudProvider() -> AIProvider {
        let provider = AIProviderSettings.selectedProvider
        return provider == .gemma4Local ? .gemini : provider
    }

    private static func initialCloudModel() -> String {
        AIProviderSettings.selectedProvider == .gemma4Local
            ? AIProvider.gemini.defaultModel
            : AIProviderSettings.selectedModel
    }

    /// Debug / UI-test hook: `-ayuvoOnboardingStep N` opens onboarding on step N so a single
    /// screen (e.g. the Apple Health step) can be exercised directly. Compiled out of release.
    private static func initialStepFromLaunchArguments() -> Int {
        #if DEBUG
        let arguments = CommandLine.arguments
        if let index = arguments.firstIndex(of: "-ayuvoOnboardingStep"), arguments.indices.contains(index + 1),
           let requested = Int(arguments[index + 1]), (0..<14).contains(requested) {
            return requested
        }
        #endif
        return 0
    ...
    private var welcomeStep: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 20) {
                ...
                Text("Meals, workouts, sleep, heart, fasting and water —\nprivate, on your device.")
    ...
    private var notificationsStep: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 24) {
                ...
                VStack(spacing: 12) {
                    Text("Ayuvo would like to send you Notifications")
                        .font(.system(.subheadline, design: .rounded, weight: .medium))
                        .multilineTextAlignment(.center)
                    Divider()
                    HStack {
    ...
    /// Visible, pre-checked Coach consent. Stored only when the user confirms with
    /// Continue after Health authorization succeeds — never flipped silently.
    @State private var coachHealthConsent = true
        ...
        VStack(spacing: 0) {
            ...
            VStack(spacing: 20) {
                ...
                VStack(spacing: 8) {
                    ...
                    Text("Keep steps, heart, sleep, workouts and body\nmeasurements in sync — kept on this device.")
                        .font(.system(.callout, design: .rounded))
                        .foregroundStyle(.secondary)
                        .multilineTextAlignment(.center)
                }
            ...
            VStack(spacing: 12) {
                // Consent card: same 24 pt side margins as the Continue button below, caption
                // wraps instead of pushing the switch off the trailing edge.
                Toggle(isOn: $coachHealthConsent) {
                ...
                Button {
                    Task {
                        ...
                        if authorized {
                            ...
                            // Coach consent is the value of the visible toggle the user just
                            // confirmed with Continue (stamps coachHealthDataConsentedAt).
    ...
    private var aiProviderSubtitle: String {
        String(localized: "Use your own AI provider key, or run a private model right on this iPhone.")
    }

    // MARK: Setup mode chooser

    private var aiSetupModeChooser: some View {
        HStack(spacing: 12) {
            aiSetupModeCard(
                .cloud,
                icon: "key.fill",
                title: String(localized: "Your API key"),
                subtitle: String(localized: "Gemini, OpenAI, Groq…")
            )
            aiSetupModeCard(
                .onDevice,
                icon: "iphone",
                title: String(localized: "On-device"),
                subtitle: String(localized: "Runs on this iPhone · no key")
            )
        }
    }

    private func aiSetupModeCard(_ mode: AISetupMode, icon: String, title: String, subtitle: String) -> some View {
        let isSelected = aiSetupMode == mode
        return Button {
            guard aiSetupMode != mode else { return }
            withAnimation(.snappy) { aiSetupMode = mode }
        } label: {
                    ...
                    Image(systemName: icon)
                        ...
                        .background {
                            if isSelected {
                                ...
                                Circle().fill(AppColors.calorie.opacity(0.12))
                            }
                        }
                    Spacer(minLength: 0)
            ...
            .overlay {
                RoundedRectangle(cornerRadius: 16)
                    .stroke(isSelected ? AppColors.calorie : Color.clear, lineWidth: 2)
            }
        ...
        .accessibilityIdentifier("onboarding.aiMode.\(mode.rawValue)")
    }

    // MARK: On-device setup

    private var onDeviceSetupCard: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack(spacing: 12) {
                Image(systemName: "cpu")
                    ...
                    .background(AppColors.calorie.opacity(0.12), in: RoundedRectangle(cornerRadius: 10))
                VStack(alignment: .leading, spacing: 2) {
                    Text("Gemma 4 E2B")
                        .font(.system(.headline, design: .rounded))
                    Text("Private AI that runs on this iPhone")
                        .font(.system(.caption, design: .rounded))
                        .foregroundStyle(.secondary)
                }
            }
            ...
            if gemmaManager.isEligible {
                Gemma4ModelSettingsView(onAvailabilityChange: {})

                if !hasStartedOnDeviceModel {
                    Text("Start the download to continue.")
                        .font(.system(.caption, design: .rounded, weight: .medium))
                        .foregroundStyle(AppColors.calorie)
                        .accessibilityIdentifier("onboarding.onDevice.downloadHint")
                }
            } else {
                HStack(alignment: .top, spacing: 10) {
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        Text("Not available on this iPhone")
                            .font(.system(.subheadline, design: .rounded, weight: .semibold))
                        Text("Gemma 4 needs an iPhone with at least 8 GB of RAM. Choose “Your API key” to continue.")
                            .font(.system(.caption, design: .rounded))
                            .foregroundStyle(.secondary)
                            .fixedSize(horizontal: false, vertical: true)
                    }
                ...
                .accessibilityIdentifier("onboarding.onDevice.unsupported")
            }
            ...
            HStack(alignment: .top, spacing: 10) {
                Image(systemName: "apple.logo")
                    ...
                    .foregroundStyle(.secondary)
                    .frame(width: 24)
                Text("On supported iPhones, Apple Intelligence also handles typed and voice meals without a key.")
                    .font(.system(.caption, design: .rounded))
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
        ...
        .padding(16)
        .background(AppColors.appCard, in: RoundedRectangle(cornerRadius: 16))
        .accessibilityIdentifier("onboarding.onDevice.card")
    }

    /// True once the Gemma download has at least started (or the model is already installed).
    private var hasStartedOnDeviceModel: Bool {
        switch gemmaManager.state {
        case .downloading, .verifying, .preparing, .downloaded, .ready, .generating:
            true
    ...
    private var aiPrivacyNoticeCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            aiNoticeRow(
                icon: "photo.fill",
                title: "AI analysis",
    ...
    private var aiTermsCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 6) {
                ...
                Text("and")
                    .foregroundStyle(.secondary)
                Link("Terms of Service", destination: AppLinks.termsURL)
            }
    ...
    private var aiProviderContinueButton: some View {
        Button {
            ...
            Text(String(localized: "Accept & Continue"))
                .font(.system(.body, design: .rounded, weight: .semibold))
                .foregroundStyle(.white)
                .frame(maxWidth: .infinity)
                .frame(height: 54)
    ...
    /// Step 11 primary CTA: terms required plus a usable BYOK key/model (cloud), or an
    /// eligible iPhone whose Gemma download has at least started (on-device, no key needed).
    private var canAdvanceAI: Bool {
        guard hasAcceptedTerms else { return false }
        if aiSetupMode == .onDevice {
            return gemmaManager.isEligible && hasStartedOnDeviceModel
        }
        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            HStack {
                ...
                Picker("", selection: $byokProvider) {
                    ForEach(AIProvider.visionProviders.filter { $0 != .gemma4Local }) { provider in
    ...
    private func completeAIChoiceAndAdvance() {
        ...
        case .cloud:
            ...
            AIProviderSettings.selectedProvider = byokProvider
            AIProviderSettings.selectedModel = byokModel
        case .onDevice:
            // Applied now if Gemma is already prepared, otherwise when the download finishes.
            AIProviderSettings.pendingLocalModelSelection = true
            AIProviderSettings.applyPendingLocalModelSelectionIfPossible(isSelectable: gemmaManager.isSelectable)
        }
        aiConsentGiven = true
