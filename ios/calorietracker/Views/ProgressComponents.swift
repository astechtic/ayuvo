...
func downsampled(_ points: [TrendPoint], maxPoints: Int = 60) -> [TrendPoint] {
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            if weightEntries.isEmpty {
                ...
                .chartOverlay { proxy in
                    ChartScrubOverlay(proxy: proxy, points: chartPoints, date: { $0.date }, selected: $inspectedPoint) { point in
                        weightInspectorLabel(for: point)
                    }
                }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if entries.isEmpty {
                ...
                .clipped()
                .chartOverlay { proxy in
                    ChartScrubOverlay(proxy: proxy, points: plottedPoints, date: { $0.date }, selected: $inspectedPoint) { point in
                        bodyFatInspectorLabel(for: point)
                    }
                }
                .animation(.snappy(duration: 0.16), value: inspectedPoint?.date)
            }
