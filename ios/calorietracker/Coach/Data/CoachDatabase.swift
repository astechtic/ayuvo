...
/// Single-connection SQLite actor for Coach (`coach.sqlite`, schema v1 from
/// `shared/coach/schema.sql`). Same shape as `MedicationsDatabase` and `RecordsDatabase`: WAL,
/// `synchronous=NORMAL`, `foreign_keys=ON`, `busy_timeout=5000`, `PRAGMA user_version` +
/// `coach_meta.schema_version`. A separate database from the health mirror, records and medications
/// so their parity tests are untouched.
actor CoachDatabase {
    ...
    nonisolated static func open(
        ...
        targetVersion: Int = CoachSchema.schemaVersion
    ) async throws -> CoachDatabase {
        try CoachLocation.prepareDirectory(url.deletingLastPathComponent(), fileManager: fileManager)
        let connection = try HealthDBConnection(path: url.path, readOnly: false, fileProtection: true)
        let database = CoachDatabase(connection: connection, url: url)
        try await database.configure(targetVersion: targetVersion)
        return database
    }

    /// An unreadable file is moved aside (`coach.sqlite.corrupt-<ms>` + sidecars) and a fresh
    /// database is created. Attachment blobs are untouched; their rows are gone, so they are
    /// collected by the next sweep.
    nonisolated static func openQuarantiningCorruption(
        url: URL,
        fileManager: FileManager = .default
    ) async throws -> (CoachDatabase, quarantined: URL?) {
        do {
    ...
    nonisolated static func inMemory(targetVersion: Int = CoachSchema.schemaVersion) async throws -> CoachDatabase {
        let connection = try HealthDBConnection(path: ":memory:", readOnly: false, fileProtection: false)
        let database = CoachDatabase(connection: connection, url: nil)
        try await database.configure(targetVersion: targetVersion)
        return database
    }
    ...
    /// v1 (`schema.sql`) then every migration in order, each step in its own transaction that also
    /// stamps `user_version` + `coach_meta.schema_version`.
    private func migrate(to targetVersion: Int) throws {
        var version = try userVersion()
        if version < CoachSchema.baseVersion, targetVersion >= CoachSchema.baseVersion {
            try connection.inTransaction {
                for statement in CoachSchema.statements {
                    try connection.exec(statement)
                }
                try stampVersion(CoachSchema.baseVersion)
            }
            version = CoachSchema.baseVersion
        }
        for migration in CoachSchema.migrations where migration.version > version && migration.version <= targetVersion {
            try connection.inTransaction {
    ...
    func meta(_ key: String) throws -> String? {
        try connection.scalarText("SELECT value FROM coach_meta WHERE key=?", [.text(key)])
    }

    func setMeta(_ key: String, _ value: String) throws {
        try connection.run("INSERT OR REPLACE INTO coach_meta (key, value) VALUES (?, ?)", [.text(key), .text(value)])
    }
    ...
    /// Deletes every row (schema and `schema_version` stay). Used by "Delete all chats" when the file
    /// itself cannot be removed. The caller deletes the attachment blobs.
    func wipeAllData() throws {
        try connection.inTransaction {
            ...
            DELETE FROM coach_meta WHERE key <> 'schema_version';
            """)
        }
    }
}
