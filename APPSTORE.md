...
Multi-turn chat that can pull your food log, workouts, weight trend, fasts and — only with your explicit toggle — your Health data. Use Google Gemini, OpenAI, Anthropic, xAI, OpenRouter, Together AI, Groq, Hugging Face, Fireworks AI, DeepInfra, Mistral, DeepSeek, Cerebras, Ollama or any OpenAI-compatible endpoint with your own key, or run Gemma 4 E2B fully on-device. Apple Intelligence can handle text food descriptions on supported iPhones. Keys stay in the iOS Keychain; requests go straight from your phone to the provider.
...
PRIVACY
No account, no ads, no analytics, no tracking. Everything you log stays on your device. Data leaves your phone only when you act: sending a request to the AI provider you chose, scanning a barcode, browsing exercise images, downloading a model, or turning on your own iCloud backup (which never includes Health data or coach chat). Export everything, delete everything — Settings → Delete All Data.
