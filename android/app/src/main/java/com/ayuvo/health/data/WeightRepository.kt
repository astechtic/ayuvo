...
import com.ayuvo.health.services.health.ExternalWeight
...
class WeightRepository(
    ...
    suspend fun addEntry(entry: WeightEntry): WeightGoalReachedEvent? {
        ...
        if (profile != null && goal != null && previousLatest != null) {
            val crossed = when (profile.goal) {
                com.ayuvo.health.models.WeightGoal.LOSE ->
                    previousLatest.weightKg > goal && entry.weightKg <= goal
                com.ayuvo.health.models.WeightGoal.GAIN ->
                    previousLatest.weightKg < goal && entry.weightKg >= goal
                com.ayuvo.health.models.WeightGoal.MAINTAIN -> false
            }
    ...
    suspend fun deleteEntry(id: UUID) {
        ...
        // a surviving ayuvo-tagged record would resurrect through the own-record
     ...
     * manual entries (random ids) are preserved. Records Ayuvo itself wrote restore
